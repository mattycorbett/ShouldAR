...
namespace OpenCVForUnityExample
{
    public class FaceDetector : MonoBehaviour
    {
        ...
        protected virtual void Start()
        {
            ...
            _videoTextureRgb = new Texture2D(captureWidth, captureHeight, TextureFormat.RGBA32, false);
        ...
        void RawVideoFrameAvailable(MLCamera.CameraOutput output, MLCamera.ResultExtras extras, MLCameraBase.Metadata metadataHandle)
        {
            if (output.Format == MLCamera.OutputFormat.RGBA_8888)
            {
                ...
                ConvertMLImageToTexture2D(ref _videoTextureRgb, output.Planes[0]);
                //UpdateRGBTexture(ref _videoTextureRgb, GetComponent<Renderer>());
            }
        }
        ...
        protected void Update()
        {
            ...
            bgrMat = new Mat(size, CvType.CV_8UC3);
            Utils.texture2DToMat(_videoTextureRgb, rgbaMat);
            ...
            UpdateRGBTexture(ref texture, _screenRendererRGB);

        }

        protected void postprocess(Mat frame, List<Mat> outs, Net net, int backend = Dnn.DNN_BACKEND_OPENCV)
        {
        ...
        private void UpdateRGBTexture(ref Texture2D videoTextureRGB, Renderer renderer)
        {
            renderer.materials[0].mainTexture = videoTextureRGB;
        }


        private void ConvertMLImageToTexture2D(ref Texture2D videoTextureRGB, MLCamera.PlaneInfo imagePlane)
        {
